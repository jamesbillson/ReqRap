	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update','projectcreate','upload'),
				'users'=>array('@'),
			),
...
public function actionCreate($id)
	{
		...
		if(isset($_POST['Diary']))
		{
			...
			if($model->save())
			$this->redirect(array('/req/project/diary','id'=>$model->project_id));
		}
   ...
   public function actionUpload($id)
    {
        if(isset($id) && Diary::model()->findByPk($id)){
            ...
            {   
                ...
                $file_name = Utils::uniqueFile($model->picture->name);

                if($model->picture->saveAs($path.$file_name)){
                    
                    $thumb = $path.'thumbs/'.$file_name;

                    $image = new EasyImage($path.$file_name);
                    $image->resize(450, 450);
                    $image->save($path.'thumbs/'.$file_name);
                    

                    //persist into database
                    $photo = new Photo;
                    $photo->generation_id = $id;
                    ...
                    if($photo->save(false)){
                        ...
                        $data[] = array(
                            ...
                            'url' => Yii::app()->params['photo_folder'].$photo->file,
                            'thumbnail_url' => Yii::app()->params['photo_folder'].'thumbs/'.$photo->file,
                            'delete_url' => Controller::createUrl('photo/delete',array('id' => $photo->id, 'method' => 'uploader')),
                            'delete_type' => 'POST'
                        );
                    }
                } else {
