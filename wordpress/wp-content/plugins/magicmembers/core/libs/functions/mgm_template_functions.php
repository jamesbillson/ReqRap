...
function mgm_user_subscription_info($user_id=NULL,$args=array()) {	
	...
	$html .= '
		...
		<div class="row ' . ($row_ctr++ % 2 == 0 ? 'alternate' : '') . ' br_bottom">
			...
			<div class="cell width2 padding10px"><strong>:</strong></div>
			<div class="cell width73 padding10px">' .((is_super_admin() ? 'N/A' : mgm_stripslashes_deep(esc_html(mgm_get_user_membership_type($user->ID)))) .' (<a href="'. mgm_get_custom_url('transactions',false,array('action' => 'upgrade', 'username' => $user->user_login, 'upgrade_prev_pack' => $pack_id)) . '">' . __('Upgrade','mgm') . '</a>)').'</div>
		</div>
