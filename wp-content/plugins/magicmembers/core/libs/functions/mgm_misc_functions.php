...
function mgm_user_has_access($post_id = false, $allow_on_purchasable = false) {
	...
	if ($post_id) {		
		...
		if ($allow_on_purchasable && $purchasable) {// if purchasable
			...
			if ((array_diff($allowed_membership_types, $user_membership_types) != $allowed_membership_types) ||
				(!empty($default_access) && (array_diff($allowed_membership_types, $default_access) != $allowed_membership_types) )
			) { // if any match found
				// set access
				$return = true;
				// check hide content
...
function mgm_get_currencies(){
	...
	$currencies = array('AED' => sprintf(__('%s - Arab Emirates Dirham','mgm'), 'AED'),'ARS' => sprintf(__('%s -  Argentina Peso ','mgm'), 'ARS'),
		...
		'MXN' => sprintf(__('%s - Mexican Peso','mgm'), 'MXN'),'MYR' => sprintf(__('%s - Malaysian Ringgit','mgm'), 'MYR'),
...
function mgm_get_all_userids() {
	...
	if($count) {
		...
		add_option('mgm_userids', $result, '', 'no');
	}
	return $result;
...
function mgm_update_userids($user_id, $action = 'insert') {
	if ($user_id) {
		...
		if ($action == 'insert') {
			...
			// Make sure the ID is not already saved
			elseif (FALSE === in_array($userids, $user_id)) {
				$userids[] = $user_id;
			}
...
function mgm_replace_email_tags($message,$user_id=NULL) {
	...
	$tags = array('name','username','password','login_url','admin_email','email','blogname',
				  ...
				  'amount','membership_type','currency_sign','date');
	// loop
	foreach($tags as $tag){
		// check
		if(!isset(${$tag})) ${$tag} = '';
...
function mgm_default_access_membership_types(){
	...
	foreach ($subscription_packs as $subscription_pack) {			
		//check
		if($subscription_pack['default_access']){
			//init
